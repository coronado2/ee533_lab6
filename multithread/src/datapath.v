...
module datapath (
    ...
    // ID (Decode)
	 ...
	 genvar r;
	 generate
    for (r = 0; r < 4; r = r + 1) begin : REGFILE_ARRAY
        regfile u_regfile (
            .clk(clk),
            .rst_n(rst_n),
            .wena(wregen_wb && (thread_id_wb == r)),
            .waddr(wreg1_wb),
            .wdata(write_data),
            .r0addr(reg1_id),
            .r1addr(reg2_id),
            .r0data(r0data_thread[r]),
            .r1data(r1data_thread[r])
        );
    end
	 endgenerate
	 
	 assign r1out_id = r0data_thread[thread_id_id];
	 assign r2out_id = r1data_thread[thread_id_id];
	 
	 pipeline_reg #(.REGS(2+`PC_WIDTH+1+1+1+1+`DATA_WIDTH+`DATA_WIDTH+`REG_ADDR_WIDTH+2+4+1+`DATA_WIDTH)) id_ex_stage(
		.clk(clk),
		.rst_n(rst_n),
		.en(1'b1),
		.D({thread_id_id, pc_id, wregen_id, wmemen_id, mem_to_reg_id, br_en_id, r1out_id, r2out_id, wreg1_id, cond_id, alu_op_id, alu_src_id, imm_id}),
		.Q({thread_id_ex, pc_ex, wregen_ex, wmemen_ex, mem_to_reg_ex, br_en_ex, r1out_ex, r2out_ex, wreg1_ex, cond_ex, alu_op_ex, alu_src_ex, imm_ex})
	 );
